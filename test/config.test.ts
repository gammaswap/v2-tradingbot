...
describe("price configuration validation", () => {
  ...
  it("rejects an invalid capital exposure percentage", () => {
    ...
    ).toContain("MAX_CAPITAL_EXPOSURE_PERCENT must be between 0 and 100");
  });
  ...
  it("rejects an invalid logit half-spread", () => {
    ...
    ).toContain("LOGIT_HALF_SPREAD must be between 0.005 and 1");
  });
  ...
  it("requires quote slots to be an integer between 1 and 100", () => {
    ...
    expect(validateRiskConfiguration({ ...base, LEVELS_PER_SIDE: 0 } as never)).toContain(
      "LEVELS_PER_SIDE must be an integer between 1 and 100",
    );
    expect(validateRiskConfiguration({ ...base, LEVELS_PER_SIDE: 2.5 } as never)).toContain(
      "LEVELS_PER_SIDE must be an integer between 1 and 100",
    );
    expect(validateRiskConfiguration({ ...base, LEVELS_PER_SIDE: 101 } as never)).toContain(
      "LEVELS_PER_SIDE must be an integer between 1 and 100",
    );
  });
  ...
  it("validates quote-size concavity", () => {
    ...
    ).toContain("QUOTE_SIZE_CONCAVITY must be greater than 0 and at most 10");
    expect(
      validateRiskConfiguration({
        ...base,
  ...
  it("requires contract exposure percentage to be between 1 and 100", () => {
    ...
    ).toContain("MAX_CONTRACT_EXPOSURE_PCT must be between 1 and 100");
    expect(
      validateRiskConfiguration({
        ...base,
