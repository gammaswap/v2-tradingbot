...
export function initializePeriodLength(state: RuntimeState, assetId: bigint): void {
  ...
  if (state.periodLength !== null && state.periodLength !== periodLength) {
    throw new Error(`asset periodLength changed from ${state.periodLength} to ${periodLength}`);
  }
