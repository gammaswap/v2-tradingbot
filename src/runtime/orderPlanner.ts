...
export function planOrders(
  ...
  for (let i = 0; i < minLength; i++) {
    ...
    if (!permitted) {
      replacementSize = capOrderSizeByMargin(isBuy, oldOrder.size, price, collateral + oldMargin);
      ...
      permitted =
        replacementMarginChange <= 0 ||
        canPlaceOrder(isBuy, replacementSize, price, collateral + oldMargin);
    }
