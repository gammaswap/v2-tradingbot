...
import {
  ...
  protocolValueToSafeNumber,
} from "../utils/protocolMath.js";
import {
  maxSizeForMargin,
  PROTOCOL_MIN_PRICE,
  PROTOCOL_MAX_PRICE,
...
export function hasFreshBook(book: BookSnapshot | null = STATE.book): boolean {
  ...
  return STATE.bookUpdatedAtMs > 0 && nowMs() - STATE.bookUpdatedAtMs <= CFG.BOOK_STALE_MS;
}

export function shouldPauseForFairValue(book: BookSnapshot | null = STATE.book): boolean {
  const freshOracleAvailable = hasFreshFairValue();

  if (CFG.USE_ORACLE_FAIR_VALUE && CFG.REQUIRE_FRESH_FAIR_VALUE && !freshOracleAvailable) {
    return true;
  }
...
export function shouldCancelReplace(
  ...
  if (Math.abs(o.price - newPrice) <= tol && Math.abs(o.size - newSize) <= tolSize) {
    return false;
  }
...
export function calculateRemainingEpochSeconds(
  ...
  const rawRemaining = asset.expiration > nowSeconds ? asset.expiration - nowSeconds : 0n;
  const boundedRemaining = rawRemaining > periodLengthBigInt ? periodLengthBigInt : rawRemaining;
...
function bucketRemainingEpochSeconds(remainingSeconds: number, periodLength: number): number {
  ...
  return Math.min(periodLength, Math.ceil(remainingSeconds / bucket) * bucket);
}
...
export function calculateTotalSizes(
  ...
  const { periodLength, remainingSeconds } = calculateRemainingEpochSeconds(asset, timestampMs);
  const bucketedRemainingSeconds = bucketRemainingEpochSeconds(remainingSeconds, periodLength);
  const tOverT = bucketedRemainingSeconds / periodLength;
  const H_0 = (STATE.accountBalance * CFG.MAX_CONTRACT_EXPOSURE_PCT) / 100;
  const H_t =
    H_0 * Math.pow(1 - Math.pow(1 - tOverT, CFG.TOTAL_SIZE_DECAY_K), CFG.TOTAL_SIZE_DECAY_A);

  const currentInventory = STATE.invBase;
  const inventoryTarget = CFG.INV_TARGET;
  const requestedBidSize = Math.max(0, inventoryTarget + H_t - currentInventory);
  const requestedAskSize = Math.max(0, currentInventory - inventoryTarget + H_t);
  ...
  return {
    bidSize: roundDownToOrderLot(Math.min(requestedBidSize, H_0, maximumBidSize)),
    askSize: roundDownToOrderLot(Math.min(requestedAskSize, H_0, maximumAskSize)),
  };
}

export function calculateCurrentRiskAversion(asset: Asset, timestampMs = Date.now()): number {
  const { periodLength, remainingSeconds } = calculateRemainingEpochSeconds(asset, timestampMs);
...
export function calculateInventorySkew(
  ...
  return gamma * inventory * normalizedReferencePrice * (1 - normalizedReferencePrice);
}

export function depthToWipe(
  book: BookSnapshot,
  side: Side,
  levels: number,
...
export function nearestOrderAtPrice(
...
export function buildEquidistantLadderPrices(
  ...
  for (let i = 0; i < CFG.LEVELS_PER_SIDE; i++) {
    const fraction = CFG.LEVELS_PER_SIDE === 1 ? 0 : i / (CFG.LEVELS_PER_SIDE - 1);
    const bidPrice = roundToTick(bidStart + (bidEnd - bidStart) * fraction, "buy");
    const askPrice = roundToTick(askStart + (askEnd - askStart) * fraction, "sell");
...
export type LadderPriceModel = (typeof LADDER_PRICE_MODEL)[keyof typeof LADDER_PRICE_MODEL];
...
export function distributeTotalSizeAcrossLadder(
  ...
  const bestPrice = side === "buy" ? Math.max(...targetPrices) : Math.min(...targetPrices);
  ...
  const weights = distances.map((distance) => {
    const normalizedDistance = farthestDistance === 0 ? 0 : distance / farthestDistance;
    return Math.pow(1 + normalizedDistance, CFG.QUOTE_SIZE_CONCAVITY);
  });
...
export function availableCollateral(): number {
  const reserveLimitedCollateral = Math.max(0, STATE.baseBal - CFG.BASE_RESERVE_MIN);
  const percentageLimitedCollateral =
    (Math.max(0, STATE.baseBal) * CFG.MAX_CAPITAL_EXPOSURE_PERCENT) / 100;

  return Math.min(reserveLimitedCollateral, percentageLimitedCollateral);
}
...
export function capOrderSizeByMargin(
  ...
  const maxSize = maxSizeForMargin(isBuy ? "buy" : "sell", price, marginBudget);
...
export function chooseAggressionSide(book: BookSnapshot, reference: number): Side | null {
  switch (CFG.AGGRESSION_MODEL) {
    ...
    case "edge-with-fallback":
      return (
        chooseFairValueEdgeSide(book, reference) ?? chooseMeanReversionAggressionSide(reference)
      );
  }
}
