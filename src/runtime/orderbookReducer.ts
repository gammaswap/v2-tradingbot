...
export type MarketApplyResult =
  "applied" | "duplicate" | "ignored" | "ignored-epoch" | "buffered" | "invalid";
...
export function installBookSnapshot(state: LocalOrderBookState, snapshot: BookSnapshot): boolean {
...
export function applyMarketUpdate(
  ...
  if (update.seqId <= state.seqId) {
    logger.warn(
      "[buffered] update.seqId <= state.seqId >> update.seqId:",
      update.seqId,
      "state.seqId:",
      state.seqId,
    );
  ...
  if (update.seqId !== state.seqId + 1n) {
    logger.warn(
      "[buffered] update.seqId != state.seqId + 1 >> update.seqId:",
      update.seqId,
      "state.seqId:",
      state.seqId + 1n,
    );
...
function addSnapshotOrders(state: LocalOrderBookState, orders: PendingOrder[], side: Side): void {
...
function applyPayload(state: LocalOrderBookState, update: WebSocketMarketUpdate): boolean {
...
function applyOrder(state: LocalOrderBookState, update: WebSocketOrderUpdate): boolean {
...
function applyTrade(state: LocalOrderBookState, update: WebSocketTradeUpdate): boolean {
...
function applyCancel(state: LocalOrderBookState, update: WebSocketCancelUpdate): boolean {
...
function addOrderToLevel(state: LocalOrderBookState, order: PendingOrder): void {
  const levels = order.side === "buy" ? state.bidsByPrice : state.asksByPrice;
...
function removeOrderFromLevel(state: LocalOrderBookState, order: PendingOrder): void {
  const levels = order.side === "buy" ? state.bidsByPrice : state.asksByPrice;
...
function levelSnapshots(levels: TreeMap<number, MutablePriceLevel>): BookLevel[] {
