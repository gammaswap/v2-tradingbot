import { type TrackedOrderIntent, type IntentOutcome } from "./orderIntent.js";
import {
  RUNTIME_ORDER_INTENTS as ORDER_INTENTS,
  RUNTIME_QUOTE_COOLDOWNS as QUOTE_COOLDOWNS,
} from "./context.js";
...
export function handleCancelReplaceResponse(
  ...
  if (status === "SUCCESS") {
    ...
    startQuoteCooldown(
      intent,
      `replacement failed: ${data.replacement?.reason ?? data.reason ?? "unknown"}`,
    );
