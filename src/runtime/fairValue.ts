...
export function computeFairValueEstimate(
  spot: bigint,
  asset: Asset,
  timestampMs = nowMs(),
  ...
  if (!Number.isFinite(spotNumber) || !Number.isFinite(strikeNumber) || strikeNumber <= 0)
    return null;
...
export function updateFairValueFromOracle(
  spot: bigint,
  oracleTs: bigint | null = null,
): FairValueEstimate | null {
...
function roundToProtocolTick(value: number): number {
  ...
  return clamp(Math.round(value / tick) * tick, PROTOCOL_MIN_PRICE, PROTOCOL_MAX_PRICE);
}
