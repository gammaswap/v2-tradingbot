...
let activeEnvironment: NodeJS.ProcessEnv = process.env;

function envNum(name: string, def: number): number {
  const v = activeEnvironment[name];
...
function envApiUrl(): string {
  const explicit = activeEnvironment.API_URL;
...
export function validateProductionConfig(env: NodeJS.ProcessEnv = process.env): string[] {
  ...
  const errors: string[] = [];
  errors.push(...validatePriceConfiguration(config));
  errors.push(...validateOrderSizeConfiguration(config));
  errors.push(...validateRiskConfiguration(config));
...
/** Creates a complete configuration from an explicitly supplied environment. */
...
/** Stable defaults used by imported TradingBot instances. */
export const DEFAULT_CFG = createConfigFromEnvironment({});

/** Environment-derived configuration retained for the repository's standalone runner. */
export const CFG = createConfigFromEnvironment(process.env);
