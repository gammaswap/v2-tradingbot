...
export function assertProtocolPrice(price: number | bigint, label = "price"): void {
...
export function assertProtocolSize(size: number | bigint, label = "size"): void {
...
export function assertProtocolOrder(
  ...
  const effectivePrice = side === "buy" ? priceValue : PROTOCOL_SCALE - priceValue;
  const margin = (sizeValue * effectivePrice) / PROTOCOL_SCALE;

  if (margin < PROTOCOL_MIN_ORDER_MARGIN) {
    throw new Error(
      `order margin ${margin} is below protocol minimum ${PROTOCOL_MIN_ORDER_MARGIN}`,
    );
  }
}
...
export function maxSizeForMargin(
  ...
  assertProtocolPrice(price);
  if (
    typeof marginBudget === "number" &&
    (!Number.isSafeInteger(marginBudget) || marginBudget < 0)
  ) {
