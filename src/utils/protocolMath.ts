...
export function protocolMulDiv(
    ...
    return (
        toProtocolBigInt(left, "left") *
        toProtocolBigInt(right, "right")
    ) / denominator;
}

export function protocolNotionalBigInt(
    size: number | bigint,
    price: number | bigint,
): bigint {
...
export function protocolNotional(
    size: number | bigint,
    price: number | bigint,
): number {
    return protocolValueToSafeNumber(
        protocolNotionalBigInt(size, price),
        "protocol notional",
    );
}
