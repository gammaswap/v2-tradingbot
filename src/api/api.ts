...
import { RUNTIME_CFG as CFG, RUNTIME_STATE as STATE, type Side } from "../runtime/context.js";
import type {
  ApiBalancesResponse,
  ApiAssetResponse,
  ApiBookResponse,
...
export function getConfiguredContracts(): ExchangeContractsInput | undefined {
  ...
  if (isNonZeroAddress(CFG.DEPOSIT_LEDGER_ADDRESS))
    contracts.depositLedger = CFG.DEPOSIT_LEDGER_ADDRESS;
...
function normalizePendingOrders(
  orders: any[],
  side: Side,
  epoch: number,
...
function normalizePending(data: any, address: string, epoch: number): ApiPendingResponse {
  return {
    ...
    buys: normalizePendingOrders(data.buys ?? [], "buy", epoch).map((order) => ({
      ...order,
      account: order.account || address,
    })),
    sells: normalizePendingOrders(data.sells ?? [], "sell", epoch).map((order) => ({
      ...order,
      account: order.account || address,
    })),
  };
...
export async function apiGetBook(epoch: bigint | number): Promise<ApiBookResponse> {
  const data = unwrapData(
    await getInfoClient().getOrderBook({ assetId: CFG.ASSET_ID, epoch: epoch.toString() }),
  );
...
export async function apiGetAssetAtEpoch(epoch: bigint | number): Promise<ApiAssetResponse> {
  const data = unwrapData(
    await getInfoClient().getAssetAtEpoch({
...
export async function apiGetPosition(epoch: bigint | number): Promise<ApiPositionResponse> {
  const data = unwrapData(
    await getInfoClient().getPosition({
...
export async function apiGetClaimable(epoch: bigint | number): Promise<ApiClaimableResponse> {
  const data = unwrapData(
    await getInfoClient().getClaimable({
      account: STATE.account,
      assetId: CFG.ASSET_ID,
      epoch: epoch.toString(),
    }),
...
export async function apiGetPending(
  address: string,
  epoch: bigint | number,
): Promise<ApiPendingResponse> {
  const data = unwrapData(
    await getInfoClient().getBookOrders({
      assetId: CFG.ASSET_ID,
      epoch: epoch.toString(),
      account: address,
    }),
...
export async function apiCancelOrder(
  ...
  const res =
    orderHash.toLowerCase() === ZeroHash.toLowerCase()
